<template>
  <UiCard class="flex flex-col h-full max-h-[80vh] shadow-xl rounded-b-none rounded-t-lg">
    <!-- Modal de disclaimer -->
    <HealthDisclaimerModal :is-open="showDisclaimerModal" @update:open="showDisclaimerModal = $event"
      @accepted="handleDisclaimerAccepted" />

    <!-- En-tête du chatbot -->
    <UiCardHeader class="flex items-center justify-between p-4 bg-blue-50 rounded-t-lg border-b">
      ...
      <div class="flex items-center space-x-2">
        <Button @click="showDisclaimerModal = true" variant="ghost" size="sm" class="text-gray-500 hover:text-gray-700"
          title="Voir les limitations et disclaimers">
          <Icon name="lucide:info" class="w-4 h-4" />
        </Button>
        <Button @click="clearConversation" variant="ghost" size="sm" class="text-gray-500 hover:text-gray-700"
    ...
    <UiCardContent>
      ...
      <div ref="messagesContainer" class="flex-1 overflow-y-auto p-4 space-y-4 min-h-0 max-h-[500px] overflow-auto">
        <div v-for="message in messages" :key="message.id" class="flex"
          :class="message.role === 'user' ? 'justify-end' : 'justify-start'">
          <div class="max-w-[80%] p-3 rounded-lg" :class="getMessageClasses(message)">
            ...
            <div v-if="message.role === 'assistant'" class="flex items-start space-x-2">
              ...
              <div class="flex-1">
                <div class="prose prose-sm max-w-none" v-html="formatMessageContent(message.content)" />
            ...
            <div v-if="message.type === 'emergency'"
              ...
              <span class="text-xs text-red-800 font-medium">Message d'urgence</span>
            </div>

            <!-- Indicateur d'avertissement -->
            <div v-if="message.type === 'warning'"
              ...
              <span class="text-xs text-yellow-800 font-medium">Conseil important</span>
            </div>

            <!-- Indicateur de vérification -->
            <div v-if="message.isVerified && message.role === 'assistant'"
              ...
              <span class="text-xs text-green-800 font-medium">Information vérifiée</span>
            </div>
            ...
            " class="mt-3 space-y-2">
              ...
              <div class="flex flex-wrap gap-2">
                <Button v-for="action in message.suggestedActions" :key="action"
                  @click="handleSuggestedAction(action, message.type)" :variant="message.type === 'emergency' ? 'destructive' : 'outline'
                    " size="sm" class="text-xs">
                  <Icon :name="getSuggestedActionIcon(action)" class="w-3 h-3 mr-1" />
                  {{ action }}
                </Button>
              </div>
            </div>
      ...
      <div class="px-4 py-2 bg-yellow-50 border-t border-yellow-200" data-testid="permanent-disclaimer">
        <div class="flex items-start space-x-2">
          <Icon name="lucide:info" class="w-4 h-4 text-yellow-600 mt-0.5 flex-shrink-0" />
  ...
  <div class="p-4 border-t bg-gray-50 rounded-b-lg">
    <form @submit.prevent="handleSubmit" class="flex space-x-2">
      <Textarea v-model="currentMessage" placeholder="Posez votre question sur la santé..."
        class="flex-1 min-h-[44px] max-h-32 resize-none" :disabled="isLoading"
        @keydown.enter.exact.prevent="handleSubmit" @keydown.enter.shift="() => { }" rows="1" />
      <Button type="submit" :disabled="!currentMessage.trim() || isLoading" class="self-end">
        <Icon :name="isLoading ? 'lucide:loader-2' : 'lucide:send'" :class="['w-4 h-4', isLoading && 'animate-spin']" />
      </Button>
    </form>
    ...
    <div v-if="messages.length === 1" class="mt-3">
      ...
      <div class="flex flex-wrap gap-2">
        <Button v-for="suggestion in quickSuggestions" :key="suggestion" @click="selectSuggestion(suggestion)"
...
import type { ChatMessage } from "~/composables/health/useHealthChatbot";
